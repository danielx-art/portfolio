export default function vec (x = 0, y = 0, z = 0): vector {
  const self: vector = {
    x,
    y,
    z,
    ...
    rotate: function (a: number, o: vector = vec(0, 0, 0)) {
      ...
      const newx = (self.x - o.x) * Math.cos(a) - (self.y - o.y) * Math.sin(a);
      const newy = (self.x - o.x) * Math.sin(a) + (self.y - o.y) * Math.cos(a);
      self.x = newx;
      self.y = newy;
      // const mag = self.mag();
